...
// Update the camera's aspect ratio and frustum size when the window resizes
window.addEventListener('resize', () => {
  camera.left = window.innerWidth / -2;
  camera.right = window.innerWidth / 2;
  camera.top = window.innerHeight / 2;
  camera.bottom = window.innerHeight / -2;
  camera.updateProjectionMatrix(); // Update the camera's projection matrix after resizing
});
...
document.querySelector("#retry").addEventListener("click", () => {
  ...
  isDead = false; // 🆕 reset
  score = 0; // Reset score
  highestY = 0; // Reset highest Y position
});
...
//   if (isDead) return; // 🆕 block input when dead
//   if (event.keyCode == "38") {
//     move("forward");
...
let highestY = 0; // Since chicken starts at y=0

function updateScore() {
  if (isDead) return; // 🆕 stop scoring after death
  if (chicken.position.y > highestY) {
    highestY = chicken.position.y;
    score++;
    console.log("Score updated:", score);
    // Send score update for real-time UI display
...
function move(direction) {
  ...
  if (direction === "forward") {
    ...
    addLane();
    updateScore();  // Increment the score when moving forward
    ...
    if (
      ...
      return;
    if (!stepStartTimestamp) startMoving = true;
    updateScore();  // Increment the score when moving backward
  ...
  if (
    ...
    lanes[currentLane].vechicles.forEach((vechicle) => {
      ...
      if (chickenMaxX > carMinX && chickenMinX < carMaxX) {
        ...
        isDead = true; // 🆕 set dead
        // Send final score to parent on death
